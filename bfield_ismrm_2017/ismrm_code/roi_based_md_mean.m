function [out1,out2,out3,out4] = roi_based_md_mean(j)
            ...
            for y = 1:93
                for z = 1:75
                    ...
                    if(rois_mask.img(x,y,z) == 2)
                        roi_two = [roi_two,results(j,i).md1(x,y,z)];
                    end

                    if(rois_mask.img(x,y,z) == 3)
                        roi_three = [roi_three,results(j,i).md1(x,y,z)];
                    end

                    if(rois_mask.img(x,y,z) == 4)
                        roi_four = [roi_four,results(j,i).md1(x,y,z)];
                    end
                end
            end
        end
