function out = report_dev_sf_number_peaks_two_fiber
    overall_output = [];
    for i = 1:11
        fa_file = 'gold_fa.nii';
        ...
        [variable.(my_field_1)] = sf_number_peaks_two(fa_file,gold_file,peaks_file);
    ...
    a1 = errorbar(sf_mean_matrix(:,1),sf_mean_matrix(:,2),sf_std_matrix(:,2),'linewidth',1.5); M1 = sprintf('No. of peaks - 2');
    ...
    ylabel('SF for two fibers','fontweight','demi','fontsize',12);
    legend([a1],M1);
    title('Success Fraction for Two fiber case for 11 samples');
