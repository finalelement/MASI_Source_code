...
function [mean_mean_matrix,mean_std_matrix] = peak_to_peak_two_fiber_error_box_report_deviation_nuero(mask)
    ...
    for i = 1:11
        ...
        [variable.(my_field_1),variable.(my_field_2)] = peak_to_peak_error_method2strength_two_fiber(fa_file,peaks_gold,peaks_file,mask);
    ...
    %title('PAS Error with Gold PAS Two Fiber Case');
    
    % Cutting out the zeros
    mean_mean_matrix = mean_mean_matrix(mean_mean_matrix(:,2) ~= 0,:);
    mean_std_matrix = mean_std_matrix(mean_std_matrix(:,2) ~= 0,:);
    
    % Cutting out FA > 1
    mean_mean_matrix = mean_mean_matrix(mean_mean_matrix(:,1) <= 1,:);
    mean_std_matrix = mean_std_matrix(mean_std_matrix(:,1) <= 1,:);
end
