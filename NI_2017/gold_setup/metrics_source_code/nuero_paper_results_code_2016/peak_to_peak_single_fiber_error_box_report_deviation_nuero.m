...
function [mean_mean_matrix,mean_std_matrix] = peak_to_peak_single_fiber_error_box_report_deviation_nuero(mask)
    ...
    %mean_plot = errorbar(mean_mean_matrix(:,1),mean_mean_matrix(:,2),mean_std_matrix(:,2),'--','linewidth',1,'Color','red');
    ...
    % Cutting out the zeroes
    mean_mean_matrix = mean_mean_matrix(mean_mean_matrix(:,2) ~= 0,:);
    mean_std_matrix = mean_std_matrix(mean_std_matrix(:,2) ~= 0,:);
    
    % Cutting out FA greater than 1.
