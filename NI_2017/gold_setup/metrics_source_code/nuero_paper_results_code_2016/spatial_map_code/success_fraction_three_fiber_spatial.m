% Three fiber case, Success fraction for a single set referencing to the gold
% set

% Define Success Fraction for three fiber case. The peak number of the test
% set must match to the gold which is 3 and the angular tolerance of the
% corresponding fibers should not be more than 5/10/15 degrees. Will plot
% graphs for all three on account of gold set's FA.

function [final_output,suc_spa] = success_fraction_three_fiber_spatial(fa,peaks_gold,peaks_test,tolerance,mask)
            ...
            for z = 1:dims(3)
                ...
                if (AAA(x,y,z,1) == 0)
                    ...
                    if(mask_mask.img(x,y,z) == 1)
                        % Validating only three+ fiber voxels from gold set
                        if (AAA(x,y,z,3) >= 3)
                            ...
                            if (BBB(x,y,z,3) >= 3)
                                ...
                                if (angle_tolerances(1,1) <= tolerance && angle_tolerances(1,2) <= tolerance && angle_tolerances(1,3) <= tolerance)
                                    s_voxels = s_voxels + 1;
                                    row_2 = [frac_val 1];
