...
function report_dev_single_fiber_sf(tolerance)
    overall_output = [];
    for i = 1:12
        ...
        [variable.(my_field_1)] = success_fraction_one_fiber(fa_file,gold_file,peaks_file,tolerance);
    ...
    figure(1)
    hold on;
    a1 = errorbar(sf_mean_matrix(:,1),sf_mean_matrix(:,2),sf_std_matrix(:,2),'linewidth',1.5); M1 = sprintf('tolerance-%d',tolerance);
