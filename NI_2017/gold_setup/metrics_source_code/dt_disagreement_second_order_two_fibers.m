...
function [output,mean_output] = dt_disagreement_second_order_two_fibers(fa,dt_file,peaks_file,mask)
                ...
                if (AAA(x,y,z,1) == 0)
                    ...
                    if(mask_mask.img(x,y,z) == 1)  
                        ...
                        if (AAA(x,y,z,3) == 2)
                            ...
                            angle_error3 = atan2d(norm(cross(v,i)),dot(v,i));
                            if (angle_error3 > 90)
                                angle_error3 = 180 - angle_error3;
                            end

                            angle_error4 = atan2d(norm(cross(v,j)),dot(v,j));
                            if (angle_error4 > 90)
                                angle_error4 = 180 - angle_error4;
                            end

                            if (angle_error1 < angle_error2)
                                second_error = angle_error4;
                            else
                                second_error = angle_error3;
                            end
